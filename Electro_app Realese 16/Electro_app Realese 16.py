"""
This is Electro app realese 15
Here we add  a new module "Logout using session"
"""
...
Electro_app.secret_key = "My Secret password"
Electro_app.config["SESSION_TYPE"] = "filesystem"
Session(Electro_app)
# --------------------
...
def my_validate_page():
    ...
    if len(my_db_result)==0:
         return "First register to login.......! <a href='/login'>Login</a>   "
    ...
    if len(my_db_result) > 0:
         #to store username in session object
         session['username'] = entered_username
         return "Login sucessful......!   <a href='/'>Home</a>"
    else:
        return "Login Failed. Invalid Credentials....!  <a href='/login'>Go Back To Login</a>"
...
def my_register_page():
    ...
    if entered_password_1 != entered_password_2:
        return "Both Passwords Are Not Matching.....<br><a href='/login'>Go Back To Registration</a>"
...
# --------------------
# END POINT - 10: http://127.0.0.1:5000/view_devices URL MAPPED to '/view_devices'
# --------------------
@Electro_app.route('/view_devices', methods=['GET'])
def my_view_devices_page():
# --------------------
        ...
        print("Executing select query")
        my_db_cursor.execute('SELECT * FROM newdevice_table')
        print("Done")
        ...
        print(" Obtaining Profit/loss from the data")
        my_db_result_with_profit = []
        for DEVICENAME, DEVICEMODEL, DEVICEID, DEVICEMANFCOST, DEVICESELLINGCOST, OTHERCOST,REMARKS in my_db_result :
            PROFIT = DEVICESELLINGCOST-DEVICEMANFCOST-OTHERCOST
            row_with_profit = (DEVICENAME, DEVICEMODEL,DEVICEID, DEVICEMANFCOST,DEVICESELLINGCOST,OTHERCOST,PROFIT,REMARKS)
            my_db_result_with_profit.append(row_with_profit)

        # All the data is in my_db_result_with_profit
        return flask.render_template('View_devices.html', my_data=my_db_result_with_profit)
# --------------------
# END POINT - 11 : http://127.0.0.1:5000/logout URL MAPPED to '/logout'
# --------------------
@Electro_app.route('/logout')
def my_logout_page():
